...
#
# Tags look like `v<version>-<release>`, which is the shape every role in this
# fleet publishes. This repository is at v1.0.0-0.
#
# This role deploys no software: it is host-level housekeeping with no container
# image, no upstream project and therefore no version of its own.
# `defaults/main.yml` holds nothing a version could be read out of - a size in
# megabytes, a path, a list of kernel parameters - and Renovate has no leaf here
# to bump. The `1.0.0` in the tags is a number somebody picked by hand.
#
...
# it. Before this existed the repository had no tags at all: mash-playbook
# pinned it by raw commit hash, and four years of fixes sat on `main` unreleased.
...
# the repositories that consume this role - mash-playbook,
# matrix-docker-ansible-deploy and etke.cc/ansible all pin it by tag.
#
# `files` and `templates` do not exist here today. They are listed so that the
# day somebody adds one, its content is already release-defining.
role_defining_paths=(
	'defaults'
	'files'
	'meta'
